"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.Details = void 0;
/*!
 * Copyright (C) 2019-2020 Silas B. Domingos
 * This source code is licensed under the MIT License as described in the file LICENSE.
 */
const Class = require("@singleware/class");
const RestDB = require("@singleware/restdb");
const Types = require("../types");
/**
 * Order details, entity class.
 */
let Details = class Details extends Class.Null {
};
__decorate([
    RestDB.Schema.Null(),
    RestDB.Schema.String(0, 64),
    Class.Public()
], Details.prototype, "orderId", void 0);
__decorate([
    RestDB.Schema.Null(),
    RestDB.Schema.Enumeration(Object.values(Types.Mode)),
    Class.Public()
], Details.prototype, "mode", void 0);
__decorate([
    RestDB.Schema.Null(),
    RestDB.Schema.Enumeration(Object.values(Types.Currency)),
    Class.Public()
], Details.prototype, "orderCurrency", void 0);
__decorate([
    RestDB.Schema.Null(),
    RestDB.Schema.Integer(0, 999999999999),
    Class.Public()
], Details.prototype, "orderEffectiveAmount", void 0);
__decorate([
    RestDB.Schema.Null(),
    RestDB.Schema.Integer(0, 999999999999),
    Class.Public()
], Details.prototype, "orderTotalAmount", void 0);
Details = __decorate([
    RestDB.Schema.Entity('Order/Details'),
    Class.Describe()
], Details);
exports.Details = Details;
//# sourceMappingURL=details.js.map